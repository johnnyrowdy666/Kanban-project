    ...
    >
      ...
      <div className="flex items-start justify-between mb-3">
        <div
          ...
          {...attributes}
          {...listeners}
          className="font-medium text-gray-900 text-sm line-clamp-2 flex-1 cursor-grab active:cursor-grabbing"
        >
          {task.title}
        </div>
...
const SortableTaskList: React.FC<SortableTaskListProps> = ({
  ...
  const { isOver, setNodeRef } = useDroppable({
    id: `task-list-${tasks[0]?.columnId || 'empty'}`,
    data: {
      type: 'task-list',
      columnId: tasks[0]?.columnId,
    },
  });
  ...
  return (
    <div 
      ref={setNodeRef}
      className={`space-y-3 min-h-[100px] ${isOver ? 'bg-blue-50 rounded-lg p-2' : ''}`}
    >
      {tasks.map((task) => (
        <SortableTask
          key={task.id}
